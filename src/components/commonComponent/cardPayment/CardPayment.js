...
export default function CardPayment({user_id, handleSaveNewCard, setOpen}) {
    ...
    const handleChange = (e) => {
        ...
        switch (name) {
            case 'cardNumber':
                setCardData({...cardData, [name]: value.replace(/[^\dA-Z]/g, '').replace(/(.{4})/g, '$1 ').trim()});
                break;
            case 'cardExpiry':
                setCardData({...cardData, [name]: value.replace(/[^\dA-Z]/g, '').replace(/(.{2})/g, '$1 ').trim()});
                break;
            default:
    ...
    const handleSaveCard = () => {
        ...
        if (cardNumber.length < 19 || cardExpiry.length < 5 || cardCVC.length < 3 || cardHolderName.length < 3) {
        ...
        <div className="addCard">
            ...
            <div className="cardBody">
                ...
                <Button fullWidth className={`${!disabled && 'createCard'} createCardBtn`} disabled={disabled}
                        variant="contained" onClick={() => handleSaveCard()}>
                    Save Card
